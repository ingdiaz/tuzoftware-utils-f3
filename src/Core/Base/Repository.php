...
class Repository
{
    ...
    protected function existResult(){
        $results=$this->db->exec($this->sql,$this->filters);
        $this->filters=array();
        if(empty($results)){
            return false;
    ...
    protected function addLikeFilter($filters,$parameterName,$conjunction="AND"){
        if(!empty($filters[$parameterName])){
            $this->sql= $this->sql." $conjunction $parameterName LIKE :$parameterName";
            $this->filters["$parameterName"]='%'.$filters["$parameterName"].'%';
        }
    }

    protected function addEqualFilter($filters,$parameterName,$conjunction="AND"){
        if(!empty($filters[$parameterName])){
